...
function loadJson(path) {
  return fs.readFile(`data/4/${path}.json`).then(data => JSON.parse(data))
}
...
async function processWeapon({ value: node }) {
  ...
  if(wpn.character === 'ranger') {
    ...
    if(wpn.category === 'guide') {
      ...
      if(wpn.name.endsWith('(Artillery Squad)')) {
        wpn.subCategory = 'artillery'
      } else if(wpn.name.endsWith('(Attack Whale)')) {
        wpn.subCategory = 'whale'
      } else if(wpn.name.endsWith(')')) {
